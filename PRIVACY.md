...
| `b` (browser preview) on a Markdown file, once the page loads | `fonts.googleapis.com` / `fonts.gstatic.com` | Fetched by your browser for the paper-note fonts (Caveat for headings, Patrick Hand for body, Klee One for CJK) | Don't press `b` on Markdown, or block the hosts |
